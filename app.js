...
(function () {
  ...
  function renderHero(article) {
    ...
    return `
      <article class="hero" data-article-id="${escapeHtml(id)}">
    ...
    try {
      ...
      data.articles.forEach((article) => {
        ...
        if (article.image || article.video) {
          // only replace if the slot is genuinely empty (placeholder) —
          // do NOT replace if an img/video is already present, even if it
          // hasn't finished loading yet: replacing it mid-load wipes a
          // working image off the screen
          const hasMedia = slot.querySelector("img.article-media, video.article-media");
          if (!hasMedia) {
            slot.innerHTML = renderMedia(article, isHero);
          }
        }
      });
